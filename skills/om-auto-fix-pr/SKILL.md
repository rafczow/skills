...
**ALWAYS check first:** Apply `.ai/skills/om-auto-fix-pr/SKILL.md` when present; safety rules still win.
...
- **Fork supersede/credit**: when the review step carries a fork PR forward into a replacement PR, preserve the `Supersedes {prRef}` line, credit the original author, and reassign the replacement to them — per `om-auto-review-pr`'s fork flow and the Supersede Credit Rule checks in `references/pr-finalize.md`.
