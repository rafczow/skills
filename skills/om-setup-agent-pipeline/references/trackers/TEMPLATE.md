...
- **Resolvable-discussions merge gate.** Some trackers let a project require every review thread to be marked resolved before merge — and can auto-mark the automation's *own* posted comments (claim, label-rationale, completion, …) as part of that same gate, so a skill that only ever posts and never resolves can permanently block its own merge. This has no dedicated bullet elsewhere in this contract because not every tracker has the concept, but every tracker author should explicitly ask: *does this tracker have a resolvable-thread (or equivalent) merge precondition, and if so, does it ever apply to comments this descriptor itself posts?* If yes, document a `resolve-discussions`-style operation (or equivalent step folded into **merge-pr**'s preflight) that resolves the automation's own threads — never a human reviewer's — before every merge attempt. `gitlab.md`'s `resolve-discussions` operation is a worked example.
- **Claim/lock protocol.** Skills coordinate via three claim signals: assignee = automation user, `in-progress` marker, and a `🤖`-prefixed claim comment with a timestamp. Define how each is expressed in this tracker; all three should be readable back by **get-issue**/**get-pr** so concurrent skills detect the lock. The `ci-monitoring` marker is deliberately **not** one of them: it records that a finished, fully reported run still owes a CI-result comment, and must never be read as a lock. **Carve-out for trackers where authors routinely self-assign:** the literal rule ("any assignee other than the automation user means locked") assumes assignment is a deliberate, uncommon action — true on GitHub, but not universal. If this tracker's "assignee" field is commonly set by authors on their own issues/PRs as a matter of course (e.g. a UI default), the literal rule flags most human-authored items as locked and forces a stop-and-ask on nearly every invocation. State an explicit position here rather than silently inheriting the GitHub default: either document a tracker-specific carve-out to the assignee signal, or explicitly accept the friction and say why.
...
**Per-type ID namespaces are a correctness trap, not just a style question.** Some trackers (GitHub)
number issues and PRs from one shared sequence, so a bare `#123` unambiguously identifies one object.
Others (GitLab: `#123` issues vs `!123` merge requests; Jira: per-project prefixed keys) number each
object type independently, so the same number can refer to two different things depending on the
sigil/prefix used. Any skill or shared logic that pattern-matches a generic `#\d+` and assumes it has
found a PR reference is wrong on a tracker with per-type namespaces. State this tracker's identifier
scheme explicitly here, including the exact sigil/prefix used for each object type, so cross-tracker
logic doesn't inherit GitHub's single-namespace assumption by accident.
