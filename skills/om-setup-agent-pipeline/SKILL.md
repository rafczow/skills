...
Every skill in this collection checks, right after loading the config, for a repo-local extension of the same name at `.ai/skills/<skill-name>/SKILL.md`. This skill does not create local skills; it only owns the convention. Full contract: `references/agentic-setup.md`. Install paths: `references/skill-install-locations.md`.
...
3. **Ask the user (skip with `--defaults`).** First check for an existing agent instruction file to determine greenfield vs existing-project mode. Then confirm validation, tracker (default `github`), browser provider, label mode, QA gate, spec path, review checklist, and missing project docs (reconciliation mode on existing projects). Guidance: `references/interview-questions.md`.
...
7. **Generate the project docs.** Per the Project docs section above, generate every doc the user opted into — each only when it does not already exist:
   ...
   On an existing-project setup, apply the chosen reconciliation mode instead of generating independently — `references/project-docs.md` → Existing-project reconciliation. Show each doc to the user before writing; never overwrite an existing process doc or agent instruction file.
