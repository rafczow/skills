...
4. **Process each `(pr, issue)` pair.** Fetch the issue state first: run **get-issue** for {issue} on `$REPO`, requesting `number,state,title,url,labels,assignees,comments`.
   ...
   **4c. Closed without merge.** Post the closed-without-merge informational comment from `references/report-templates.md` via **comment-issue**; do **not** close. When a different merged PR in the same window declares `Supersedes {prRef}`, link it via the template's `supersededBySuffix`.
